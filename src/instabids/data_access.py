...
async def create_project(task_id: str, payload: dict) -> None:
    await supabase().table("projects").insert({"id": task_id, "payload": json.dumps(payload)}).execute()

async def get_project_status(task_id: str) -> dict[str, Any] | None:
    res = await supabase().table("projects").select("status,bids").eq("id", task_id).single().execute()
