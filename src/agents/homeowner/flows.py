...
REQUIRED_FIELDS = [
    ...
    "location_description", "timeline"
]

# Define states globally or pass them around if needed by helpers outside the build function
states = [
    "START", "GATHER_TITLE", "GATHER_DESCRIPTION", "GATHER_PROJECT_TYPE",
    "GATHER_CATEGORY", "GATHER_LOCATION", "GATHER_TIMELINE",
    ...
    "CONFIRM_DETAILS",
    "AWAIT_CONFIRMATION", "HANDLE_CORRECTION",
    "DONE", "FAILED"
]
...
def _all_required_fields_gathered(gathered_data: Dict) -> bool:
    ...
    return all(field in gathered_data and gathered_data.get(field) for field in REQUIRED_FIELDS)
...
def _find_next_gather_state(gathered_data: Dict) -> str:
    ...
    for field in REQUIRED_FIELDS:
        if not gathered_data.get(field):
            ...
            if gather_state in states: return gather_state
            else:
                 logger.error(f"Logic error: Field '{field}' has no valid GATHER state mapped.")
                 return "FAILED"

    # Then check optional fields before confirmation
    if "desired_outcome_description" not in gathered_data: return "GATHER_DESIRED_OUTCOME"
    if "allow_group_bidding" not in gathered_data: return "CONFIRM_GROUP_BIDDING"
    if gathered_data.get("current_photos_handled") is not True: return "HANDLE_CURRENT_PHOTOS"
    if gathered_data.get("inspiration_photos_handled") is not True: return "HANDLE_INSPIRATION_PHOTOS"

    # If all required and optional steps done, go to confirm
    return "CONFIRM_DETAILS"


def build_project_creation_flow(llm_service: Llm, memory_service: Optional[Memory] = None) -> LLMFlow:
    """
        ...
        "GATHER_DESIRED_OUTCOME": """
         ...
         "AWAIT_CONFIRMATION": """
            ...
            **Output Format:** Respond ONLY with a valid JSON object:
            ...
            """.format(required_fields_list=REQUIRED_FIELDS),
         "HANDLE_CORRECTION": """
             You are the InstaBids Homeowner Agent.
             Current Gathered Data: {gathered_data}
             User Input: {user_message}
    ...
    def state_mapper(current_state: str, llm_output: str, gathered_data: Dict) -> Tuple[str, Dict, Optional[Union[str, Dict]]]:
        """
        ...
        """
        logger.debug(f"State Mapper - Current: {current_state}, LLM Output: {llm_output}")
        ...
        try:
            ...
            for key, value in extracted.items():
                 ...
                 if value or isinstance(value, bool): # Update if value is truthy or boolean false
            ...
            if next_state_suggestion in states:
                 ...
                 if current_state == "HANDLE_CORRECTION" and next_state_suggestion.startswith("GATHER_") and next_state_suggestion in states:
                      next_state = next_state_suggestion
                 else:
                      logger.warning(f"LLM suggested invalid next state: {next_state_suggestion}. Staying in {current_state}.")
            ...
            if current_state == "GATHER_TITLE":
                 ...
                 if not title or len(str(title).split()) > 10 or len(str(title).split()) < 1 or str(title).lower() in ['yes', 'no', 'ok', 'okay']:
                      logger.warning(f"Title extraction failed or invalid in state {current_state}. Asking again.")
                      next_state = "GATHER_TITLE"
                 ...
                 elif next_state == "GATHER_TITLE": # If valid but LLM didn't suggest moving on
                 ...
                 if not desc or len(str(desc).split()) < 3:
                      logger.warning(f"Description extraction failed or too short in state {current_state}. Asking again.")
                      next_state = "GATHER_DESCRIPTION"
                      prompt_to_user = "Could you please provide a bit more detail about the project?"
                 ...
                 valid_types = ['one-time', 'recurring', 'repair', 'handyman', 'labor-only', 'multi-step']
                 if not proj_type or proj_type not in valid_types:
                      logger.warning(f"Project type extraction failed or invalid in state {current_state}. Asking again with options.")
                      next_state = "GATHER_PROJECT_TYPE"
                      prompt_to_user = {
                          "text": "Sorry, I didn't catch that. What type of project is this?",
                          "quick_replies": [
                 ...
                 if not category:
                      logger.warning(f"Category extraction failed in state {current_state}. Asking again.")
                      next_state = "GATHER_CATEGORY"
                      # Refined re-prompt based on project type
                      ptype = updated_data.get('project_type', 'project')
                      examples = "(e.g., Roofing, Plumbing, Painting)"
                      if ptype == 'repair': examples = "(e.g., Plumbing, Electrical, Drywall, Appliance)"
                      elif ptype == 'recurring': examples = "(e.g., Lawn Care, Cleaning, Pool Service)"
                ...
                if not zip_code or not re.match(r"^\d{5}$", str(zip_code)):
                    ...
                    next_state = "GATHER_LOCATION"
                    updated_data["last_error"] = "Invalid zip code format. Please provide a 5-digit zip code."
                    prompt_to_user = "That doesn't look like a valid 5-digit US zip code. Could you please provide the zip code for the project location?"
                 ...
                 valid_timelines = ['emergency', 'few_days_week', 'within_month', 'budgeting', 'dream_project']
                 if not timeline or timeline not in valid_timelines:
                      logger.warning(f"Timeline extraction failed or invalid in state {current_state}. Asking again with options.")
                      next_state = "GATHER_TIMELINE"
                      prompt_to_user = {
                          "text": "What's your ideal timeframe for this project?",
                          "quick_replies": [
                 ...
                 if not outcome or len(str(outcome).split()) < 2: # Basic check
                      logger.warning(f"Desired outcome extraction failed or too short in state {current_state}. Asking again.")
                      next_state = "GATHER_DESIRED_OUTCOME"
                 ...
                 if pref is None: # Check if LLM failed to extract boolean
                      logger.warning(f"Group bidding preference extraction failed in state {current_state}. Asking again.")
                      next_state = "CONFIRM_GROUP_BIDDING"
                      prompt_to_user = {
                          "text": "Sorry, I need a clear yes or no. Would you be open to grouping this job for potential discounts?",
                          "quick_replies": [{"title": "Yes", "payload": "confirm_yes"}, {"title": "No", "payload": "confirm_no"}]
                 ...
                 if next_state == "HANDLE_CURRENT_PHOTOS": # Ensure LLM didn't suggest invalid state
                      next_state = _find_next_gather_state(updated_data)

            elif current_state == "HANDLE_INSPIRATION_PHOTOS":
                 # Logic is simple: just mark as handled and move on
                 if next_state == "HANDLE_INSPIRATION_PHOTOS": # Ensure LLM didn't suggest invalid state
                      next_state = _find_next_gather_state(updated_data)
            ...
            if not validation_failed and current_state.startswith("GATHER_") and \
               next_state.startswith("GATHER_") and \
               next_state != current_state and \
               _all_required_fields_gathered(updated_data):
                 next_state = _find_next_gather_state(updated_data)
                 logger.info(f"All required fields gathered after {current_state}. Moving check to {next_state}")

            # Use the prompt from the LLM unless validation failed and set a specific one
            final_prompt = prompt_to_user if validation_failed else prompt_to_user_from_llm

            logger.debug(f"State Mapper - Final Next State: {next_state}, Updated Data: {updated_data}")
            ...
            return next_state, updated_data, final_prompt
             ...
             prompt_to_user = "I had trouble understanding that response format. Could you try again?"
