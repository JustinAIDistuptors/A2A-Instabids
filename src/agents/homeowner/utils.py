...
from ...a2a_types.core import Artifact, AgentId, TaskId, MessageId, Agent as A2aAgentInfo, ArtifactType
...
async def analyze_photo(
    ...
    if initial_context is None: initial_context = {}
    logger.info(f"Analyzing photo artifacts: {[getattr(a, 'id', 'N/A') for a in artifacts]}")
    ...
    image_artifacts = [a for a in artifacts if getattr(a, 'type', None) == ArtifactType.IMAGE]
    ...
    for artifact in image_artifacts:
        ...
        if image_uri and isinstance(image_uri, str) and image_uri.startswith("gs://"):
            ...
            valid_input = True
        elif image_uri and isinstance(image_uri, str): # Handle non-GCS URIs if needed (might require fetching)
             logger.warning(f"Artifact {identifier} has non-GCS URI '{image_uri}'. Direct content preferred or implement fetching.")
             # TODO: Optionally fetch content from non-GCS URI if needed
             continue
        else:
             logger.warning(f"Skipping artifact {identifier}: Invalid or missing image URI (gs://) or content (bytes).")
             continue
    ...
    try:
        ...
        for i, response in enumerate(batch_response.responses):
            ...
            if response.error.message:
                logger.error(f"Error analyzing photo {image_identifier}: {response.error.message}")
                analysis_results["photo_analysis"][image_identifier] = {"error": response.error.message}
                continue

            labels = [label.description for label in response.label_annotations]
            objects = [obj.name for obj in response.localized_object_annotations]
            texts = [text.description for text in response.text_annotations]
            full_text = texts[0].strip().replace('\n', ' ') if texts else "" # Clean up text

            logger.info(f"Analysis for {image_identifier}: Labels={labels}, Objects={objects}, Text='{full_text[:50]}...'")
        ...
        logger.error(f"Error calling Vision API batch_annotate_images: {e}", exc_info=True)
        ...
        for identifier in processed_identifiers:
             if identifier not in analysis_results.get("photo_analysis", {}):
                  analysis_results.setdefault("photo_analysis", {})[identifier] = {"error": f"Batch processing failed: {e}"}
    ...
    if all_objects: derived_description_parts.append(f"Detected objects include: {', '.join(list(all_objects)[:3])}.")
    if all_texts: derived_description_parts.append(f"Detected text includes: '{' | '.join(all_texts)}'.")
    if not derived_description_parts and all_labels:
         derived_description_parts.append(f"Image analysis suggests labels like: {', '.join(list(all_labels)[:3])}.")
    ...
    if derived_description_parts:
        ...
        if derived_desc.lower() not in existing_desc.lower():
            ...
            logger.info(f"Updated context description based on photos: {context['description']}")
        else:
...
async def analyze_quote(
    ...
    logger.info(f"Analyzing quote artifacts: {[getattr(a, 'id', 'N/A') for a in artifacts]}")
    ...
    quote_artifacts = [a for a in artifacts if getattr(a, 'type', None) == ArtifactType.FILE]
    ...
    for artifact in quote_artifacts:
        ...
        if not doc_uri and not doc_content:
             logger.warning(f"Skipping quote artifact {artifact_id}: Missing URI or content.")
             continue

        logger.info(f"Processing quote artifact: {artifact_id} (URI: {doc_uri is not None}, Content: {doc_content is not None})")
        try:
            extracted_text = ""
            if isinstance(ocr_service, vision.ImageAnnotatorClient):
                ...
                if doc_uri and doc_uri.startswith("gs://"):
                    ...
                    if doc_uri.lower().endswith(".pdf"): mime_type = "application/pdf"
                    elif doc_uri.lower().endswith((".png", ".jpg", ".jpeg", ".tiff")): mime_type = "image/tiff"
                    else:
                         logger.warning(f"Unsupported document type for GCS URI: {doc_uri}")
                         analysis_results["quote_analysis"][artifact_id] = {"ocr_status": "skipped_unsupported_type"}
                         continue

                    input_config = vision.InputConfig(gcs_source=gcs_source, mime_type=mime_type)
                    features = [vision.Feature(type_=vision.Feature.Type.DOCUMENT_TEXT_DETECTION)]
                    # Define output config (required for async) - results go to GCS
                    gcs_destination_uri = os.getenv("OCR_RESULT_GCS_URI", f"gs://your-ocr-results-bucket/results/{artifact_id}/") # Example
                    if "your-ocr-results-bucket" in gcs_destination_uri:
                         logger.error("OCR_RESULT_GCS_URI environment variable not set. Cannot process async OCR.")
                         analysis_results["quote_analysis"][artifact_id] = {"ocr_status": "error_config_missing"}
                         continue
                    ...
                    async_request = vision.AsyncAnnotateFileRequest(
                        requests=[{"input_config": input_config, "features": features, "output_config": output_config}]
                    )

                    loop = asyncio.get_running_loop()
                    logger.info(f"Submitting async OCR request for {doc_uri}...")
                    operation = await loop.run_in_executor(None, ocr_service.async_batch_annotate_files, request=async_request)
                    operation_name = operation.operation.name
                    logger.info(f"OCR operation started for {doc_uri}. Name: {operation_name}")
                    ...
                    logger.warning(f"Async OCR submitted for {doc_uri}. Text extraction will require polling/callback mechanism based on operation name: {operation_name}")
                    extracted_text = None # Indicate text is not yet available
                    analysis_results["quote_analysis"][artifact_id] = {"ocr_status": "pending_async", "operation_name": operation_name}
                    # --- End Async Handling ---

                elif doc_content and isinstance(doc_content, bytes):
                     logger.info(f"Processing direct content for artifact {artifact_id}...")
                     ...
                     if response and response.full_text_annotation:
                          ...
                          analysis_results["quote_analysis"][artifact_id] = {"ocr_status": "success"}
                     else:
                          logger.warning(f"No text found in direct content for artifact {artifact_id}.")
                          analysis_results["quote_analysis"][artifact_id] = {"ocr_status": "no_text_found"}
                else:
                     logger.warning(f"Unsupported quote artifact format for OCR: {artifact_id}")
                     analysis_results["quote_analysis"][artifact_id] = {"ocr_status": "skipped_unsupported_format"}
                     continue

            else:
                logger.warning(f"OCR service type {type(ocr_service)} not implemented.")

            if extracted_text: # Only append if text was successfully extracted synchronously
                full_text += extracted_text + "\n\n--- End of Document ---\n\n"

        except Exception as e:
            logger.error(f"Error during OCR for artifact {artifact_id}: {e}", exc_info=True)
            analysis_results["quote_analysis"][artifact_id] = {"error": f"OCR failed: {e}"}
    ...
    try:
        ...
        **CRITICAL: DO NOT include any numbers associated with prices, costs, totals, fees, or dollar amounts ($). Omit all monetary values.**
        ...
        logger.info("Calling LLM to extract scope/materials from quote text (price redacted).")
        # --- Call ADK LLM Service ---
        llm_response = await llm_service.predict(prompt=prompt) # Assumes predict is async
        ...
        if extracted_data.get("scope"):
            ...
            sanitized_scope = re.sub(r'\$\s?\d+([.,]\d+)?', '[PRICE REDACTED]', extracted_data['scope'])
        ...
        logger.error(f"Failed to parse LLM JSON output for quote analysis: {cleaned_llm_response}")
...
async def save_project_to_db(db: Optional[Client], details: Dict) -> Optional[str]:
    ...
    project_data = {
        ...
        "status": "open",
        "desired_outcome_description": details.get("desired_outcome_description"), # Added
        "metadata": {
    ...
    photo_paths = details.get("photo_paths", []) # Expecting list of dicts: {"path": "...", "type": "current|inspiration"}
    current_photo_data = []
    ...
    if isinstance(photo_paths, list):
        for photo_info in photo_paths:
             if isinstance(photo_info, dict) and isinstance(photo_info.get("path"), str):
                  ...
                  photo_entry = {"storage_path": photo_info["path"], "caption": photo_info.get("caption"), "photo_type": photo_type}
    ...
    try:
        ...
        if not insert_res.data:
            error_detail = getattr(insert_res, 'error', None) or getattr(insert_res, 'message', 'Unknown error')
            logger.error(f"Failed to insert project into Supabase. Detail: {error_detail}")
            return None
        ...
        if all_photo_insert_data:
            photo_res = await db.table("project_photos").insert(all_photo_insert_data).execute()
            logger.debug(f"Supabase photo insert response: {photo_res}")
            if not photo_res.data: logger.error(f"Failed to insert photos for project {project_id}. Response: {photo_res}")
            else: logger.info(f"Inserted {len(photo_res.data)} photos for project {project_id}.")
...
async def trigger_bid_card_creation(
    ...
    target_agent_info = A2aAgentInfo(id=bid_card_agent_id, name="Bid Card Agent", endpoint=bid_card_agent_endpoint)
    ...
    try:
        ...
        if created_task: logger.info(f"Successfully created task {created_task.id} for BidCardAgent.")
        else: logger.error(f"Failed to create task for BidCardAgent for project {project_id}.")
    except Exception as e:
