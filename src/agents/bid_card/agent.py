...
class BidCardAgent(AdkAgent):
    ...
    def __init__(
        ...
        agent_endpoint = os.getenv("BID_CARD_AGENT_ENDPOINT", DEFAULT_ENDPOINTS.get(AGENT_ID))
    ...
    async def handle_create_task(self, task: Task) -> None:
        ...
        try:
            ...
            photo_res = await self.db.table("project_photos").select("storage_path, caption, photo_type").eq("project_id", project_id).execute()
            ...
            bid_card_content = {
                ...
                "desired_outcome": project_data.get("desired_outcome_description"), # Added
                "current_photos": [ # Separate photo types
                    {"path": p.get("storage_path"), "caption": p.get("caption")}
                    for p in photo_data if p.get("photo_type") == "current"
                ],
                "inspiration_photos": [
                    {"path": p.get("storage_path"), "caption": p.get("caption")}
                    for p in photo_data if p.get("photo_type") == "inspiration"
                ],
            ...
            artifact_id = await self._create_bid_card_artifact(task.id, bid_card_content)

            if artifact_id:
                # 6. Update task status to COMPLETED
                task_result = {"bid_card_artifact_id": artifact_id}
                await self._update_task_status(task.id, "COMPLETED", result=task_result)
                logger.info(f"Task {task.id} completed successfully. Bid Card Artifact ID: {artifact_id}")
            ...
            error_msg = f"Error processing Bid Card creation for project {project_id}: {e}"
            logger.error(f"Task {task.id}: {error_msg}", exc_info=True)
            await self._update_task_status(task.id, "FAILED", error_message=error_msg)


    async def _create_bid_card_artifact(self, task_id: TaskId, content: Dict) -> Optional[str]:
        ...
        try:
            ...
            insert_res = await self.db.table("artifacts").insert(artifact_data).execute()
            ...
            if insert_res.data:
                ...
                logger.error(f"Failed to insert artifact {artifact_id} into Supabase. Detail: {error_detail}")
                return None
