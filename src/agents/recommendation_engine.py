...
class ProjectRecommendationEngine:
    ...
    async def get_recommendations(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            profile_res = await self.db.table("contractor_profiles") \
            ...
            query = self.db.table("projects") \
                .select("*, (SELECT COUNT(*) FROM bids WHERE bids.project_id = projects.id) as bid_count") \
                .eq("status", "open")
            ...
            if not projects_res.data:
                logger.info(f"No matching open projects found for contractor {self.contractor_id}")
                return []
            ...
            ranked_projects = sorted(scored_projects, key=lambda p: p["recommendation_score"], reverse=True)[:limit]
    ...
    async def _calculate_project_score(
        ...
        if project_type and preferred_types and project_type in preferred_types:
            type_boost = 0.2
            score += type_boost
            score_factors.append(f"Matches your preferred project type ({project_type})")
            ...
            if self.memory:
                ...
                if win_rate > 0.5:  # Good win rate
                    score += 0.1
                    score_factors.append(f"You have a {int(win_rate*100)}% success rate with this type of project")
        ...
        if location and preferred_locations and location in preferred_locations:
            ...
            if self.memory:
                ...
                if win_rate > 0.5:
                    score += 0.1
                    score_factors.append(f"You have a {int(win_rate*100)}% success rate in this location")
        ...
        if category and self.memory:
            ...
            if win_rate > 0.6:  # Very good win rate
                ...
                score_factors.append(f"You excel at {category} projects ({int(win_rate*100)}% success rate)")
    ...
    async def _record_recommendations(self, projects: List[Dict[str, Any]]) -> None:
        ...
        for project in projects:
            try:
                ...
                existing = await self.db.table("recommendation_feedback") \
                    .select("id") \
                ...
                logger.error(f"Error recording recommendation for project {project['id']}: {e}")
