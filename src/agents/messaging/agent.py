...
from ...a2a_types.core import Task, Message, Artifact, AgentId, TaskId, MessageId, Agent as A2aAgentInfo, TaskStatus
...
class MessagingAgent(AdkAgent):
    ...
    def __init__(
        ...
        agent_endpoint = os.getenv("MESSAGING_AGENT_ENDPOINT", DEFAULT_ENDPOINTS.get(AGENT_ID))
    ...
    async def handle_message(self, message: Message) -> None:
        ...
        if not project_id:
             logger.warning(f"Message {message.id}: Missing project_id context. Applying default deny.")
    ...
    async def _should_allow_message(self, project_id: Optional[str], sender_id: AgentId, recipient_id: AgentId) -> Tuple[bool, str]:
        ...
        if not project_id: return False, "Blocked: Missing project context"
        if not self.db: return False, "Blocked: Database connection unavailable"

        try:
            # Map Agent IDs to User UUIDs and Roles
            sender_user_id, sender_role = await self._get_user_id_and_role(sender_id)
            recipient_user_id, recipient_role = await self._get_user_id_and_role(recipient_id)

            if not sender_user_id or not recipient_user_id:
                 logger.warning(f"Could not map agent IDs to user roles/UUIDs for project {project_id}. Sender: {sender_id}, Recipient: {recipient_id}")
            ...
            if sender_role == 'homeowner' and recipient_role == 'contractor':
                ...
                return False, f"Blocked: Invalid communication roles ({sender_role} to {recipient_role})"
    ...
    async def _get_user_id_and_role(self, agent_id: AgentId) -> Tuple[Optional[str], Optional[str]]:
         """
         ...
         logger.warning(f"Using PLACEHOLDER naming convention for Agent ID -> User mapping for {agent_id}.")
         try:
             ...
             if len(parts) >= 3:
                 ...
                 if role_part in ['homeowner', 'contractor']:
                      ...
                      if len(uuid_part) == 36 and uuid_part.count('-') == 4:
                           logger.info(f"Simulated finding user {uuid_part} ({role_part}) based on agent ID convention.")
                           return uuid_part, role_part
         except Exception as conv_e:
             logger.error(f"Error applying naming convention to {agent_id}: {conv_e}")

         logger.error(f"Failed to map Agent ID {agent_id} to User ID/Role using any method.")
         return None, None
        ...
        try:
            ...
            for recipient_user_id in recipient_user_ids:
                 ...
                 for agent_id_key, details in AGENT_REGISTRY.items():
                      if details.get("user_id") == recipient_user_id and details.get("role") == "contractor":
                           recipient_agent_id = agent_id_key
                           break
                 # Fallback to naming convention if not in registry
                 if not recipient_agent_id:
                      recipient_agent_id = f"contractor-agent-{recipient_user_id}" # Assuming convention
                      logger.warning(f"Using naming convention to map user {recipient_user_id} to agent {recipient_agent_id}")

                 if not recipient_agent_id:
                      logger.warning(f"Could not determine agent ID for user {recipient_user_id}. Skipping broadcast.")
                      continue

                 tasks.append(self._send_broadcast_to_one(
            ...
            for i, result in enumerate(results):
                 if isinstance(result, Exception):
                      user_id_for_log = recipient_user_ids[i] if i < len(recipient_user_ids) else "unknown"
                      logger.error(f"Error broadcasting to user {user_id_for_log}: {result}")

        except Exception as e:
            logger.error(f"Error during broadcast setup for project {project_id}: {e}", exc_info=True)

    async def _send_broadcast_to_one(self, project_id: str, sender_id: AgentId, content: Any, recipient_user_id: str, recipient_agent_id: AgentId):
        ...
        if recipient_agent_info:
            try:
                ...
                logger.info(f"Broadcast message sent to {recipient_agent_id} for project {project_id}")
            except Exception as e:
    ...
    async def _get_recipient_agent_info(self, agent_id: AgentId) -> Optional[A2aAgentInfo]:
        ...
        if default_endpoint:
             ...
             return A2aAgentInfo(id=agent_id, name=f"Agent {agent_id} (Default)", endpoint=default_endpoint)
        ...
        if endpoint_from_env:
            logger.warning(f"Agent {agent_id} not in registry or defaults, using specific env var {env_var_name}.")
            return A2aAgentInfo(id=agent_id, name=f"Agent {agent_id} (Env)", endpoint=endpoint_from_env)

        # 4. If not found anywhere
        logger.error(f"Could not determine endpoint for agent {agent_id} from registry, defaults, or specific env var.")
        return None
