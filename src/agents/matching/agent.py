...
class MatchingAgent(AdkAgent):
    ...
    def __init__(
        ...
        agent_endpoint = os.getenv("MATCHING_AGENT_ENDPOINT", DEFAULT_ENDPOINTS.get(AGENT_ID))
    ...
    async def handle_create_task(self, task: Task) -> None:
        ...
        if not task_type:
            if "find contractors" in task.description.lower(): task_type = "find_contractors"
            elif "find projects" in task.description.lower(): task_type = "find_projects"
            else:
        ...
        try:
            ...
            if error_msg:
                ...
                logger.info(f"Task {task.id}: Matching process completed. Results count: {len(results)}")
                # TODO: Create result artifact if results are large or complex
                await self._update_task_status(task.id, "COMPLETED", result={"matches": results})
            else:
                 logger.warning(f"Task {task.id}: Matching process yielded no results.")
                 await self._update_task_status(task.id, "COMPLETED", result={"matches": []})
    ...
    async def find_contractors_for_project(self, project_id: str) -> Optional[List[AgentId]]:
        ...
        try:
            ...
            project_res = await self.db.table("projects").select("category, location_description").eq("id", project_id).maybe_single().execute()
            if not project_res.data:
                logger.error(f"Project {project_id} not found for matching.")
                return None
            project_category = project_res.data.get("category")
            project_location = project_res.data.get("location_description") # Assuming zip code for now

            if not project_category or not project_location:
                 logger.warning(f"Project {project_id} missing category or location for matching.")
            ...
            query = self.db.table("contractor_profiles").select("id") # Select user ID (which maps to agent ID for now)
            query = query.contains("service_categories", [project_category]) # Check if category is in the array
            # Simple zip code match - needs improvement for service areas
            query = query.eq("service_area_description", project_location) # Placeholder for area matching

            contractor_res = await query.execute()

            if not contractor_res.data:
                logger.info(f"No contractors found matching criteria for project {project_id}.")
                return []
            ...
            matching_agent_ids = [f"contractor-agent-{profile['id']}" for profile in contractor_res.data]
            logger.info(f"Found {len(matching_agent_ids)} potential contractors for project {project_id}.")
            return matching_agent_ids

        except Exception as e:
            logger.error(f"Error finding contractors for project {project_id}: {e}", exc_info=True)
            return None
        ...
        try:
            ...
            if agent_details and agent_details.get("user_id"):
                 ...
                 try:
                      ...
                      logger.warning(f"Using naming convention to map agent {contractor_id} to user {contractor_user_id}")
                 except:
                      logger.error(f"Could not map agent {contractor_id} to user UUID.")
                      return None

            # 2. Fetch contractor profile (categories, location)
            profile_res = await self.db.table("contractor_profiles").select("service_categories, service_area_description").eq("id", contractor_user_id).maybe_single().execute()
            if not profile_res.data:
                 logger.error(f"Contractor profile not found for user {contractor_user_id} (Agent: {contractor_id}).")
                 return None
            contractor_categories = profile_res.data.get("service_categories", [])
            contractor_location = profile_res.data.get("service_area_description") # Zip code

            if not contractor_categories or not contractor_location:
                 logger.warning(f"Contractor {contractor_id} missing categories or location.")
                 return []

            # 3. Fetch open projects matching criteria
            query = self.db.table("projects").select("id")
            query = query.eq("status", "open")
            query = query.contained_by("category", contractor_categories) # Project category must be one the contractor handles
            query = query.eq("location_description", contractor_location) # Simple zip match
            ...
            if not project_res.data:
                 logger.info(f"No matching open projects found for contractor {contractor_id}.")
                 return []

            matching_project_ids = [project["id"] for project in project_res.data]
            logger.info(f"Found {len(matching_project_ids)} potential projects for contractor {contractor_id}.")
            # TODO: Return Bid Card Artifact IDs instead? Requires linking projects to bid cards.
            return matching_project_ids

        except Exception as e:
            logger.error(f"Error finding projects for contractor {contractor_id}: {e}", exc_info=True)
            return None
