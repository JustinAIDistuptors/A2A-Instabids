...
class Artifact(BaseModel):
    ...
    uri: Optional[HttpUrl] = None # Optional URI if content is stored elsewhere (e.g., GCS, Supabase Storage)
    description: Optional[str] = None
...
class Message(BaseModel):
    ...
    role: MessageRole
    content: Union[str, Dict[str, Any]] # Can be simple text or structured content (e.g., tool calls)
    sender_agent_id: AgentId
    recipient_agent_id: AgentId
    artifacts: Optional[List[ArtifactId]] = None # IDs of artifacts associated with this message
    created_at: datetime.datetime = Field(default_factory=datetime.datetime.utcnow)
    metadata: Optional[Dict[str, Any]] = None
...
class Task(BaseModel):
    ...
    artifacts: Optional[List[ArtifactId]] = None # IDs of artifacts relevant to the task input/output
    messages: Optional[List[MessageId]] = None # Chronological list of message IDs in the task conversation
    result: Optional[Any] = None # Final result of the task, could be text or structured data
    metadata: Optional[Dict[str, Any]] = None
