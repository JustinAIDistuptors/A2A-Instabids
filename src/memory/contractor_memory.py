...
class ContractorMemory(PersistentMemory):
    ...
    async def load(self) -> bool:
        ...
        try:
            ...
            result = await self.db.table("contractor_bid_metrics") \
                .select("*") \
            ...
            if result.data:
                ...
                await self.db.table("contractor_bid_metrics") \
                    .insert(self._bid_metrics) \
                    .execute()
                logger.info(f"Initialized new bid metrics for contractor {self.contractor_id}")
    ...
    async def save(self) -> bool:
        ...
        if self._bid_metrics and self._is_loaded:
            try:
                ...
                if result.data:
                    logger.info(f"Saved bid metrics for contractor {self.contractor_id}")
                    return success and True
                else:
                    logger.error(f"Failed to save bid metrics for contractor {self.contractor_id}")
                    return False
    ...
    async def record_bid(self, project_id: str, bid_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if self._bid_metrics:
                ...
                self._bid_metrics["last_bid_date"] = datetime.datetime.utcnow().isoformat()
                
                # Update average bid amount
                total_amount = (self._bid_metrics["avg_bid_amount"] * (self._bid_metrics["total_bids"] - 1)) + bid_data.get("amount", 0)
                self._bid_metrics["avg_bid_amount"] = total_amount / self._bid_metrics["total_bids"]
                
                # Update bid history by project type
                project_res = await self.db.table("projects") \
                ...
                if project_res.data:
                    project_type = project_res.data.get("metadata", {}).get("project_type")
                    ...
                    if project_type:
                        if project_type not in self._bid_metrics["bid_history"]:
                            self._bid_metrics["bid_history"][project_type] = {"count": 0, "total_amount": 0}
                            
                        self._bid_metrics["bid_history"][project_type]["count"] += 1
                        self._bid_metrics["bid_history"][project_type]["total_amount"] += bid_data.get("amount", 0)
                    
                    # By category
                    if category:
                        if category not in self._bid_metrics["bid_history"]:
                            self._bid_metrics["bid_history"][category] = {"count": 0, "total_amount": 0}
                            
                        self._bid_metrics["bid_history"][category]["count"] += 1
                        self._bid_metrics["bid_history"][category]["total_amount"] += bid_data.get("amount", 0)
    ...
    async def record_bid_result(self, project_id: str, bid_id: str, status: str) -> bool:
        ...
        try:
            ...
            if self._bid_metrics:
                ...
                if project_res.data:
                    ...
                    if project_type:
                        self._update_win_rate("project_type", project_type, status == 'accepted')
                        
                    # Update by category
                    if category:
                        self._update_win_rate("category", category, status == 'accepted')
                        
                    # Update by location
                    if location:
                        self._update_win_rate("location", location, status == 'accepted')
    ...
    async def record_recommendation_reaction(self, project_id: str, reaction: str, score: Optional[int] = None) -> bool:
        ...
        try:
            ...
            result = await self.db.table("recommendation_feedback") \
                .select("*") \
                .eq("contractor_id", self.contractor_id) \
            ...
            await self.add_interaction("project_reaction", {
                "project_id": project_id,
                "reaction": reaction,
    ...
    def get_bid_preferences(self) -> Dict[str, Any]:
        ...
        if "win_rates" in self._bid_metrics and "project_type" in self._bid_metrics["win_rates"]:
            project_types = self._bid_metrics["win_rates"]["project_type"]
            
            # Filter to types with at least 3 bids
            valid_types = {k: v for k, v in project_types.items() if v.get("bids", 0) >= 3}
            
            # Calculate win rates
            type_win_rates = {k: v.get("wins", 0) / v.get("bids", 1) for k, v in valid_types.items()}
            
            # Get top 3 by win rate
            preferred_types = sorted(type_win_rates.items(), key=lambda x: x[1], reverse=True)[:3]
            preferences["preferred_project_types"] = [t[0] for t in preferred_types if t[1] > 0.3]  # Only include if win rate > 30%
            
        # Extract preferred locations
        if "win_rates" in self._bid_metrics and "location" in self._bid_metrics["win_rates"]:
            locations = self._bid_metrics["win_rates"]["location"]
            
            # Filter to locations with at least 2 bids
            valid_locations = {k: v for k, v in locations.items() if v.get("bids", 0) >= 2}
            
            # Get locations with good win rates
            good_locations = [k for k, v in valid_locations.items() 
                             if v.get("wins", 0) / v.get("bids", 1) > 0.4]  # Win rate > 40%
        ...
        if "bid_history" in self._bid_metrics:
            ...
            for category, data in bid_history.items():
                if data.get("count", 0) >= 3:  # Need at least 3 bids to establish pattern
