...
class PersistentMemory(Memory):
    ...
    async def load(self) -> bool:
        ...
        try:
            ...
            result = await self.db.table("user_memories") \
                .select("memory_data") \
    ...
    async def add_interaction(self, interaction_type: str, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            await self.db.table("user_memory_interactions") \
                .insert({
                    "user_id": self.user_id,
    ...
    async def _extract_preferences(self, interaction_type: str, data: Dict[str, Any]):
        ...
        try:
            ...
            logger.error(f"Error extracting preferences for user {self.user_id}: {e}", exc_info=True)
    ...
    async def _update_preference(self, preference_key: str, value: Any, source: str):
        ...
        try:
            ...
            if preference_key not in self._memory_cache["learned_preferences"]:
                self._memory_cache["learned_preferences"][preference_key] = {"value": value, "count": 1}
            else:
            ...
            await self.db.table("user_preferences") \
                .upsert({
                    "user_id": self.user_id,
            ...
            logger.error(f"Error updating preference for user {self.user_id}: {e}", exc_info=True)
    ...
    def get(self, key: str) -> Any:
        ...
        if not self._is_loaded:
            ...
            logger.warning(f"Attempted to get key '{key}' from unloaded memory for user {self.user_id}")
            return None
    ...
    def set(self, key: str, value: Any) -> None:
        ...
        if not self._is_loaded:
            logger.warning(f"Attempted to set key '{key}' to unloaded memory for user {self.user_id}")
            return
